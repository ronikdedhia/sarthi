...
2. **`trip_planner`** — the actual orchestration/optimization engine, and the real intellectual core of the project. **Built 2026-09-10 (BUILD_PLAN.md Phase 2):**
   ...
   - **`nl_intent.py` — built 2026-09-11:** a real Gemini call (`gemini-flash-latest`, structured JSON output via `responseSchema`, not free-text parsing) turns "get me from Koramangala to T Nagar tomorrow morning, keep it cheap" into the exact `{origin, destination, cost_weight, time_weight}` shape `plan_itineraries` already takes — confirmed live end to end through the running dev server, not just against mocked tests. `POST /api/trips/plan_from_text/` is the new endpoint; the original form-based `/plan/` is untouched underneath it. This is the only place in the codebase that knows Gemini exists (plain `requests` against the real REST endpoint, no SDK dependency, deliberately — see FEASIBILITY_RESEARCH.md #5 for why this project is now wary of exotic third-party driver/SDK risk). A transient Gemini-side `503` is a real, occasionally-observed failure mode (confirmed live) — surfaced to the caller as a clear `422`, never silently retried or swallowed.
